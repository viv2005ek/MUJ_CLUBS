//loader remove


const forLoaderBody = document.body.querySelector(".forLoaderBody");
const containerLoader = document.body.querySelector(".containerLoader");

window.addEventListener("load", (e) => {
    setTimeout(() => {
        containerLoader.style.display = "none";
        forLoaderBody.classList.remove("forLoaderBody");
    }, 3000); //3 second delay
})


//typing effect
const typed = document.querySelector(".typed");
if (typed) {
    let typed_strings = typed.getAttribute("data-typed-items");
    if (typed_strings) {
        typed_strings = typed_strings.split(",");
        console.log(typed_strings);
        let typed_string = "";
        let count = 0; // Start count from 0
        let strCount = 0;
        let current_strings = typed_strings[strCount];
        let current_string_length = current_strings.length;
        let interval = setInterval(onTick, 300);

        function onTick() {
            typed_string = current_strings.substring(0, count);
            typed.innerHTML = typed_string + "|";
            count++;
            if (count > current_string_length) {
                count = 0;
                strCount++;
                // Reset strCount if it exceeds the length of typed_strings
                if (strCount >= typed_strings.length) {
                    strCount = 0;
                }
                current_strings = typed_strings[strCount];
                current_string_length = current_strings.length;
            }
        }
    }
}